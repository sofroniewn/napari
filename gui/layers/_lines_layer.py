...
class Lines(Layer):
    ...
    def _set_view_slice(self, indices):
        ...
        if len(in_slice_boxes) > 0:
            ...
            data = np.array(in_slice_boxes) + 0.5
            edge_width = self.edge_width
            data = data.reshape((-1, 2))
        else:
            # if no markers in this slice send dummy data
            data = np.empty(2,2) #np.array([[0,0],[0,0]])
            edge_width = 0

        if self.highlight and self._selected_boxes is not None:
            edge_color = np.array([Color(self.edge_color).rgba for i in range(len(data))])
            ...
            if self._selected_boxes[1] is None:
                ...
                #edge_color[2*self._selected_boxes[0]] = [0, 0.6, 1, 1]
                #edge_color[2*self._selected_boxes[0]+1] = [0, 0.6, 1, 1]
            else:
                vertex_color[self._selected_boxes[0]] = (0, 0.6, 1)
            ...
            self._node.set_data(data, width=1, color=edge_color, connect='segments')
        else:
            self._node.set_data(data, width=edge_width, color=self.edge_color, connect='segments')
